...
main.innerHTML = moviesData.Search.map(item =>{
    return `
    <div class="movie">
...
search.addEventListener('input', ()=>{
    let getMovie = moviesData.Search.filter((item)=>{
        if(item.Title.toLocaleLowerCase().includes(search.value.toLocaleLowerCase()))
            return item;
    });

    main.innerHTML = getMovie.map(item =>{
        return `
        <div class="movie">
                <img src="${item.Poster}" alt="">
                <div class="movie-info">
